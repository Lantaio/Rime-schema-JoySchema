...
# 欢喜输入方案
## 项目概述
欢喜输入方案，懂程序员心意的[Rime输入法](https://rime.im/)方案！包含一些定制化输入方案以及相关的词库。

## 设计理念
* 本输入法方案*并非*追求大而全，而是追求快速的输入体验，因此无论是中文词库还是英文词库都是选用基本词库。因为此输入法有自造词功能，用户可以在使用过程中形成自己独有的词库，这样可以有效降低拼音重码和大量无用候选项，减少翻页次数，提高输入效率。
* 本输入方案另一个重点设计理念是侧重于提高程序员中英混输的编程效率，同时也考虑了提高写Markdown文档的效率，尽量减少须要切换中/英输入法的情况，因此部分中文标点符号不直接上屏，而是提供候选菜单，可选择输入对应的中/英标点符号。对于有候选菜单的标点符号按键，基本上其对应的中文标点符号放在第1位，其对应的英文标点符号放在第2位，以方便盲打。除了‘、’号并不是按<kbd>\</kbd>键，而是连按2次<kbd>;</kbd>键，因为考虑到编程需要，按<kbd>/</kbd>键和<kbd>\</kbd>键将直接上屏英文‘/’和‘\’。部分标点符号按键还可以输入其它常用符号，具体见`joy.punctuation.yaml`，也可以根据你的喜好自定制。
