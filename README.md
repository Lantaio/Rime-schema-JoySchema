...
## 发展规划
从2023年12月16日开始，此项目将分成main和neo两个分支。虽然两个分支都是以提高输入效率为宗旨，但main分支为偏保守的传统输入理念，而neo分支则会做一些大胆的尝试。main分支的开发暂时告一段落，在Release中版本号以‘m’开头的便是基于main分支的发布版。接下来会将主要精力放在neo分支的开发上，日后在Release中版本号以‘n’开头的便是基于neo分支的发布版。两个分支的功能特点在各自的说明文档中说明，切换到相应的分支便可看到对应的Readme文档。
...
还有一个对我帮助很大的项目是[librime-lua](https://github.com/hchunhui/librime-lua)。这个项目让用户可以使用Lua脚本语言来扩展Rime输入法的功能，非常简单方便，也大大降低了我这种小白用户扩展Rime输入法的门槛，强烈推荐。
