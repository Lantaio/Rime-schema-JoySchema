...
### 霹雳更改
* ⚡️修改扩展字符集开关和预测开关的快捷键（2025/12/20）

### 优化更改
* ✨️规范化joy_cn_numerals_t.lua文件的格式（2024/8/19）
* 🔧️在已经翻页的情况下按<kbd>></kbd>键才向前翻页，否则也作为输入码，方便输入代码（2024/12/7）
* ⚙️微调<kbd>%</kbd>键标点符号列表的顺序（2025/1/16）
* 🔧️修改第1个候选项的标签为‘\_’更接近空格键的意思（2025/1/27）
* 🎨️修改候选框各个项目的颜色以更好地区分标签和候选项（2025/1/27）
* 🔧️将Ctrl+Shift+数字键触发的功能开关修改为Ctrl+Alt+数字键触发，避免与 终点输入法插件的功能冲突（2025/1/28）
* ⚙️修改个别标点候选列表以适配5.55版 finalD/终点 输入法插件（2025/2/7）
* ✨Joy English输入方案添加全角开关；添加样式来让输入体验和拼音方案一致（2025/12/22）
* ⚙️调整个别符号的列表顺序（2025/12/24）
* 🔧️恢复预测功能（默认关闭，通过快捷键开关）（2025/12/25）

### 缺陷修复
* 🛠️修复‘<’触发输入代码的缺陷（2025/1/31）
* 🛠️修改URL模式，修复个别情况不能用‘.’号翻页的问题；✍🏻️完善注释（2025/12/22）
* 🛠完善joy_prompt_s.lua中Linux/Mac目录路径提示（2026/1/15）
* 🛠️修复joy_en.schema.yaml中触发输入扩展符号的错误（2026/1/15）
* 🛠️修复joy.default.yaml中Win目录路径规则的小问题（2026/1/15）

## v2.48.209
*2024/9/17*

### 优化更改
* 🐱‍🏍优化个别标点列表以适配3.40.85版 finalD/终点 输入法插件（2024/9/17）
