# 更新日志
## v1.30.123 (2023/12/10) ← v1.0.0
### 新增功能
* 新增欢喜语句流输入方案（2023/11/25）
* 编写Lua扩展程序为触发特定规则的输入码添加提示词（2023/12/6）
* 添加许可协议的简体中文译文（2023/12/9）

### 缺陷修复
* 更改候选按键的工作方式，使其可用于输入中文数字时正常上屏候选项。**注意**，此更改会引入一个小问题，各候选按键是假设当前高亮候选项是第1候选项时才可以正常选择候选项，所以要注意不要用<kbd>↑</kbd>和<kbd>↓</kbd>键改变当前高亮候选项的位置，否则须要通过按数字键来正确选择候选项（2023/12/4）
...
* 将文档移动到JoyDoc目录以便于区分和移除（2023/12/10）
